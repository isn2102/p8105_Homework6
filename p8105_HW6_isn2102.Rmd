...
Assess RMSE to see which model is best. Pivot data to tidy, visualize distribution, and calculate the mean RMSE. Because the smallest distribution is for the prelim5 model which I initially fit, and because we can see that prelim5 also has the smallest average RMSE, we can conclude that prelim5 predicts birthweight the best in our test data.
...
Use 5000 bootstrap samples and produce an estimate for r^2 and log(beta0xbeta1) for each bootstrap sample. First create a helper function to extract log(beta0xbeta1). 
