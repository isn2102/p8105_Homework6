...
4. Look at the t test statistic for each variable to see significant ones. Also remove predictor from model and if the other parameter estimates change, keep variable in the model. Remove variables that do not change other estimates much when removing them, or with insignificant p-values.  
```{r}
...
Compare the three models by using a cross-validation approach and assessing the prediction error RMSE.
...
First fit each type of model to every row in the dataset (to every individual iteration of the sample that we randomly drew). Produce root mean square errors for each model, mapping across two things which are the linear_mod and the rest dataset. 
```{r, eval = FALSE}
cv_df = 
  cv_df %>% 
  mutate(
...
Now we actually want to understand which model is better, using the root mean square errors. Pivot data to get a tidy dataset. Then plot each one to see the distribution. The smallest distribution is for the smooth model, so we might conclude this one makes the best predictions on test data.
